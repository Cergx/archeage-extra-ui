...
export const getNowUnix: () => number = () => Math.floor(nowMs() / 1000);

/**
 * @param {number} utcMs
 * @returns {{ y: number, m: number, d: number }}
 * */
export const getMSKDatePartsFromUtcMs: (utcMs: number) => { y: number; m: number; d: number } = (utcMs) => {
...
export const formatDMY: (params: { y: number; m: number; d: number }) => string = ({ y, m, d }) => `${pad2(d)}.${pad2(m)}.${y}`;

/** @param {number} unixSec */
export const formatTimeMSK: (unixSec: number) => string = (unixSec) => {
    if (!unixSec) return '';
...
export const dayUtcMsFromUnixByTZ: (unixSec: number) => number = (unixSec) => {
...
export const getTodayUtcMsByTZ: () => number = () => {
 ...
 */
export const addDaysUtcMs: (dayUtcMs: number, deltaDays: number) => number = (dayUtcMs, deltaDays) => dayUtcMs + deltaDays * 86400000;

/**
 * @param {number} dayUtcMs
 * @returns {{ start: number, end: number }} Unix-границы дня в МСК.
 * */
export const getDayBoundsUnix: (dayUtcMs: number) => { start: number; end: number } = (dayUtcMs) => {
 ...
 */
export const getUnixForDayAtHour: (dayUtcMs: number, hourMsk: number) => number = (dayUtcMs, hourMsk) => {
 ...
 */
export const isSameDayByTZ: (aUtcMs: number, bUtcMs: number) => boolean = (aUtcMs, bUtcMs) => {
...
/** @param {number} dayUtcMs Day UTC milliseconds. */
export const isThursdayByTZ: (dayUtcMs: number) => boolean = (dayUtcMs) => {
...
export const initServerTimeOffset: () => void = () => {
...
export const syncServerTime: () => Promise<void> = async () => {
 ...
 */
export const getMSKWeekday: (utcMs: number) => number = (utcMs) => {
 ...
 */
export const getMSKTimeOfDaySeconds: (utcMs: number) => number = (utcMs) => {
 ...
 */
export const parseTime: (timeStr: string) => { hours: number; minutes: number } = (timeStr) => {
    const [h, m] = timeStr.split(':').map(Number);
    return { hours: h, minutes: m };
};

/** @returns {number} Today's weekday as zero-based Monday-first value. */
export const getTodayWeekdayMonFirst: () => number = () => {
